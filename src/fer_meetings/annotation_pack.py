...
<body>
  ...
  <div class="toolbar">
    ...
    <div class="toolbar-actions">
      <button id="saveFileBtn" type="button">Save CSV</button>
      <button id="downloadBtn" class="secondary" type="button">Download copy</button>
      <button id="resetDraftBtn" class="secondary" type="button">Clear local draft</button>
    </div>
  </div>
  <div class="status-bar">
    <span class="chip"><strong id="visibleCount">0</strong> visible rows</span>
    <span class="chip"><strong id="labeledCount">0</strong> with gold_label</span>
    <span class="chip"><strong id="dirtyCount">0</strong> local changes</span>
    <span class="chip"><strong id="saveStatus">Local draft active</strong></span>
  </div>
  <table>
    <thead>
      <tr>
  ...
  <script>
    ...
    function downloadCsv(filename = "annotation_sheet.csv") {
      ...
      elements.saveStatus.textContent = "CSV downloaded";
    }
    ...
    async function saveCsv() {
      ...
      if (window.showSaveFilePicker) {
        try {
          ...
          elements.saveStatus.textContent = "CSV saved to disk";
          return;
        } catch (error) {
    ...
    function clearDrafts() {
      ...
      elements.saveStatus.textContent = "Local draft cleared";
      renderRows();
    }
            ...
            <td class="control-cell">
              ...
              <div class="review-grid">
                <div class="review-card">
                  <div class="review-card-title">rater 1</div>
                  ${reviewBadge(row.rater_1_label)}
                </div>
                <div class="review-card">
                  <div class="review-card-title">rater 2</div>
                  ${reviewBadge(row.rater_2_label)}
                </div>
                <div class="review-card">
                  <div class="review-card-title">adjudicated</div>
                  ${reviewBadge(row.adjudicated_label)}
                </div>
                <div class="review-card">
                  <div class="review-card-title">agreement</div>
              ...
              <div class="field-row">
                ...
                <textarea
                  data-action="notes"
                  data-clip-id="${escapeHtml(row.clip_id)}"
                  placeholder="optional notes">${escapeHtml(row.notes || "")}</textarea>
              </div>
    ...
    elements.body.addEventListener("click", (event) => {
      ...
      elements.saveStatus.textContent = "Changes saved locally";
    });
    ...
    elements.resetDraftBtn.addEventListener("click", () => {
      if (confirm("This will delete the unexported local draft. Continue?")) {
        clearDrafts();
      }
    });
